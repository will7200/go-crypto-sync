...
// APIClient manages communication with the CoinBase API API v2019-11-15
...
func (c *APIClient) prepareRequest(
	...
	if c.cfg.Host != "" {
		url1.Host = c.cfg.Host
	}

	// Override request scheme, if applicable
	if c.cfg.Scheme != "" {
		url1.Scheme = c.cfg.Scheme
	}
	...
	if body != nil {
		localVarRequest, err = http.NewRequest(method, url1.String(), body)
	} else {
		localVarRequest, err = http.NewRequest(method, url1.String(), nil)
	}
	if err != nil {
		return nil, err
	}

	if strings.HasPrefix(headerParams["CB-ACCESS-SIGN"], "auto") {
		var (
			ts string
		)
		if !strings.HasPrefix(headerParams["CB-ACCESS-TIMESTAMP"], "auto") {
			ts = headerParams["CB-ACCESS-TIMESTAMP"]
		} else {
			ts = strconv.FormatInt(time.Now().Add(time.Second*3).UTC().Unix(), 10)
			headerParams["CB-ACCESS-TIMESTAMP"] = ts
		}
	...
	headerParams["CB-VERSION"] = "2019-11-15"
